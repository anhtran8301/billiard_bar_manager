...
namespace QuanLyBanBida
{
    ...
    {
        private string connect = "Data Source=LAPTOP-RMPS3KRL\\SQLEXPRESS01;Initial Catalog=QuanLyQuanBiDaFinal1;Integrated Security=True";
        ...
        #region Methods
        public void viewListTable() //Lấy data SQL xuất ra 
        {
            SqlConnection sqlConnection = new SqlConnection(connect);
            string query = "select id as [ID], name as [Name], status as [Status] from dbo.TableBida ";// khai báo câu truy  vấn để lấy thuộc tính trong SQL
            SqlDataAdapter adpter = new SqlDataAdapter(query, sqlConnection);//lấy SQl
            DataSet dt = new DataSet();
            adpter.Fill(dt, "TableBida");// lấy data đưa về dataset
        ...
        public bool kiemTraNameTable(object name)
        {
            ...
            command.Parameters.AddWithValue("@name", name);//ktra tên
            sqlConnection.Open();
        ...
        public void addTable(string name, string status)
        {
            ...
            {
                SqlConnection sqlConnection = new SqlConnection(connect);
                string query = "insert into TableBida(name,status) values (@name,@status)";// câu truy vấn

                SqlCommand cmd = new SqlCommand(query, sqlConnection);
                cmd.Parameters.Add("@name", name);
                cmd.Parameters.Add("@status", status);// nhập dữ liệu
                sqlConnection.Open();
                cmd.ExecuteNonQuery();
                sqlConnection.Close();
        ...
        public void databindingtable()//liên kết dữ liệu
        {
            SqlConnection sqlConnection = new SqlConnection(connect);
        ...
        public void searchtable(string status)//tìm bàn trống hoajce có người
        {
            SqlConnection sqlConnection = new SqlConnection(connect);
        ...
        private void dtgv_Table_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            var selectedRow = dtgv_Table.Rows[index];
            txt_idtable.Text = selectedRow.Cells[0].Value.ToString();
            txt_TableName.Text = selectedRow.Cells[1].Value.ToString();
            cb_TableStatus.Text = selectedRow.Cells[2].Value.ToString();
        }
    }
}

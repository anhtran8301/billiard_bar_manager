...
namespace QuanLyBanBida
{
    public partial class AdminFood : Form
    {
        ...
        public void databindingfood()
        {
            ...
            string query = "select f.*, c.name as categoryName from Food f left join FoodCategory c on f.idCategory = c.id";
            SqlDataAdapter adapter = new SqlDataAdapter(query, sqlConnection);
        ...
        private void dtgv_Food_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            var selectedRow = dtgv_Food.Rows[index];
            txt_idFood.Text = selectedRow.Cells[0].Value.ToString();
            txt_NameFood.Text = selectedRow.Cells[1].Value.ToString();
            cb_Category.Text = selectedRow.Cells[2].Value.ToString();
            txt_Price.Text = selectedRow.Cells[3].Value.ToString();
